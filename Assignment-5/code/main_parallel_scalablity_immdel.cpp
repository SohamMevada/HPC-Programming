...
int main(int argc, char **argv) {
    ...
    FILE *fp = fopen("speedup_vs_threads_immediate_250_100.txt", "w");
    if (fp == NULL) {
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        for (int iter = 0; iter < Maxiter; iter++) {
            ...
            clock_t start_move = clock();
            // if (iter%Maxiter == 0) {
            //     b=ident_bad_points(points,NUM_Points,dx,dy,Maxiter);
            // }
            // mover_parallel_immdt_del(points,NUM_Points,dx,dy,b);
            mover_parallel_immdt_del(points,dx,dy);
            clock_t end_move = clock();
